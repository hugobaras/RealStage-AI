...
import { assertEmailAllowed } from "../services/blacklistService.js";
    ...
    try {
      ...
      if (platform.maintenance?.enabled) {
        return res.status(503).json({
          error: platform.maintenance.message || "Maintenance en cours.",
        });
      }

      if (req.user?.email) {
        await assertEmailAllowed(req.user.email);
      }
      ...
      if (platform.flags?.modes?.[mode] === false) {
        return res.status(403).json({
          error: "Ce mode est temporairement désactivé.",
        });
      }
